...
config = {"run_name": "revision_100_noise_80",     
          "epochs":        400,
          "batch_size":    2, 
          "lr":            1e-5, 
          "image_size":    256,             
    ...
    def train(self):
        while self.epoch <= self.n_epochs: 
            ...
            if self.epoch % 5 == 0:
                ...
                print("BLUE: ORIGINAL DATA VS ORANGE: DIFFUSION MODEL GENERATED, GREEN: CONDITION")
...
class Palette(BaseModel):
    ...
    def train_step(self):
        ...
        for i, elements in enumerate(zip(self.phase_loader, self.dataloader_circ_1)):
            ...
            self.cond_image,_ ,_ ,_  = imagen.noise_schedulers.q_sample(self.cond_image.to("cpu"),torch.tensor(80))
    ...
    def test(self, use_tqdm=False):
        ...
        with torch.no_grad():
            ...
            self.cond_image,_ ,_ ,_  = imagen.noise_schedulers.q_sample(self.cond_image_.to("cpu"),torch.tensor(80))
            self.cond_image = torch.clip(self.cond_image,-1,1).to(self.device)
...
kwargs = {
    ...
    "metrics": ["mae"],
    "resume_state" : "/dss/dssfs04/lwp-dss-0002/pn49fu/pn49fu-dss-0002/ge74xuf2/diffusion/revision_100_noise_80/100",  #"", # "/dss/dssfs04/lwp-dss-0002/pn49fu/pn49fu-dss-0002/ge74xuf2/diffusion/bc_sr_once_imagen/41", # "", #  
    "n_epochs" : 100, 
...
palette_model = Palette(
    ...
    log_iter = 1000,                                                            
    model_path = "/dss/dssfs04/lwp-dss-0002/pn49fu/pn49fu-dss-0002/ge74xuf2/diffusion/revision_100_noise_80/100_Imagen.pth",    #    "", #  
...
dataloader_embed_gfdl = data.DataLoader(bc_qm_gfdl_dataset, batch_size=bs_val, shuffle=False, drop_last=True)
...
m_path="/dss/dssfs04/lwp-dss-0002/pn49fu/pn49fu-dss-0002/ge74xuf2/diffusion/revision_100_noise_80/100_Imagen.pth"   
...
if do_save_sr_dataset==True:
    ...
    for b, el in enumerate(dataloader_embed_gfdl):
        ...
        gfdl_qm,_ ,_ ,_  = imagen.noise_schedulers.q_sample(gfdl_qm_.to("cpu"),torch.tensor(80))
...
if do_save_sr_dataset==True:
    ...
    save_path = 'data/revision_gfdl_sr_noise_80_i100_e100_18_09.pth'
    save_path = '/dss/dssfs04/lwp-dss-0002/pn49fu/pn49fu-dss-0002/ge74xuf2/diffusion/revision_100_noise_80/revision_gfdl_sr_noise_80_i100_e100_18_09.pth'
